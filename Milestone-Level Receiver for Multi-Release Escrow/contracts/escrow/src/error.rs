...
pub enum ContractError {
    ...
    MilestoneHasNotApproved = 42,
    MilestoneHasAlreadyBeenReleased = 43,
    MilestoneIndexOutOfRange = 44,
}
...
impl fmt::Display for ContractError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ContractError::MilestoneHasNotApproved => {
                write!(f, "Milestone has not approved yet")
            }
            ContractError::MilestoneHasAlreadyBeenReleased => {
                write!(f, "Milestone Has Already Been Released")
            }
            ContractError::MilestoneIndexOutOfRange => {
                write!(f, "Milestone Index is out of range")
            }
        }
    }
}
