...
use crate::{
    error::ContractError,
    storage::types::{DataKey, Escrow, Milestone, Transaction},
};
...
#[inline]
pub fn list_completed_milestones(
    e: &Env,
    ...
    let mut transactions: Vec<Transaction> = Vec::new(e);

    for milestone in milestones {
        if !milestone.approved {
            return Err(ContractError::MilestoneHasNotApproved);
        } else if milestone.released {
            return Err(ContractError::MilestoneHasAlreadyBeenReleased);
        }
        let trx = Transaction {
            recipient: milestone.receiver,
            amount: milestone.amount,
            receiver_memo: milestone.receiver_memo,
        };
