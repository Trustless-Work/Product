...
use crate::storage::types::{Escrow, Milestone, EscrowType};
use crate::token::token::{Token, TokenClient};
use crate::contract::EngagementContract;
use crate::contract::EngagementContractClient;
use soroban_sdk::{testutils::{Address as _, Ledger}, vec, Address, Env, IntoVal, String};
...
fn test_initialize_time_based_escrow_fails_without_deadline() {
    ...
    assert!(result.is_err(), "Should fail when the milestone deadline is not set in time based escrow");
}
...
fn test_initialize_time_based_escrow_fails_with_past_deadline() {
    ...
    assert!(result.is_err(), "Should fail when the any milestone deadline is in past");
}
...
fn test_initialize_manual_escrow_fails_with_deadline_set() {
    ...
    assert!(result.is_err(), "Should fail for manual escrow if any deadline is set");
}