    ...
    pub fn get_escrow(e: Env) -> Result<Escrow, ContractError> {
        let mut escrow = e.storage()
            ...
            if let EscrowType::TimeBased = escrow.escrow_type {
                ...
                escrow.milestones.iter().map(|mut milestone| {
                    milestone.approved_flag = MilestoneManager::is_milestone_approved(&e, &escrow.escrow_type, &milestone);
...
fn check_escrow(e: &Env, escrow_properties: &Escrow) -> Result<(), ContractError> {
    ...
    if escrow_properties.escrow_type == EscrowType::TimeBased {
        if escrow_properties.milestones.iter().any(|m| m.approval_deadline.is_none()) {
    ...
    if escrow_properties.escrow_type == EscrowType::Manual 
        && escrow_properties.milestones.iter().any(|m| m.approval_deadline.is_some()) 
    {
