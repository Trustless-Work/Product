...
use crate::core::validators::escrow::{
    ...
    validate_initialize_escrow_conditions, validate_release_conditions, validate_withdrawal_conditions,
};
...
pub fn release_funds(
        ...
        // If receiver memo is set, only release fees and store the pending amount
        if !escrow.receiver_memo.is_empty() {
            escrow.withdrawal_state.fees_released = true;
            escrow.withdrawal_state.pending_receiver_amount = fee_result.receiver_amount;
        } else {
            // If no memo, transfer directly to receiver
...
pub fn get_escrow(e: &Env) -> Result<Escrow, ContractError> {
    ...
    pub fn withdraw_with_memo(
        ...
        let mut escrow = Self::get_escrow(e)?;
        validate_withdrawal_conditions(&escrow)?;

        let token_client = TokenClient::new(e, &escrow.trustline.address);
        let receiver = Self::get_receiver(&escrow);

        // Transfer the pending amount to receiver
        token_client.transfer(
            &e.current_contract_address(),
            &receiver,
            &escrow.withdrawal_state.pending_receiver_amount,
        );

        // Update escrow state
        escrow.flags.released = true;
        escrow.withdrawal_state.pending_receiver_amount = 0;
        e.storage().instance().set(&DataKey::Escrow, &escrow);
