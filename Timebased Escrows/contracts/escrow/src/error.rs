...
pub enum ContractError {
    ...
    FeesNotReleasedYet = 42,
    NoPendingWithdrawal = 43,
    NoMemoProvided = 44,
}
...
impl fmt::Display for ContractError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ContractError::FeesNotReleasedYet => {
                write!(f, "Fees have not been released yet")
            }
            ContractError::NoPendingWithdrawal => {
                write!(f, "No pending withdrawal amount found")
            }
            ContractError::NoMemoProvided => {
                write!(f, "No memo provided for withdrawal")
            }
        }
    }
}
